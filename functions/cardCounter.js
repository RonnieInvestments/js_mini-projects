...
function cardCounter(card) {
  ...
  if (typeof card === "number") {
    if (card>=2 && card<=6){ // Card between 2 and 6
      count += 1;
    } else if (card>=7 && card<=9) { // Neutral cards
      count += 0; 
    } else if (card === 10) { // Card number 10
      count -= 1;
    }
  }

  // Card is a string (J, Q, K, A)
  ...
  // Decide betting strategy
  if (count>0) {
    return count + " Bet";
  } else {
    return count + " Hold"; // Count less than or equal to 0
  }
}
